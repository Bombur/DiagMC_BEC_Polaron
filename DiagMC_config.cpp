...
DiagMC::DiagMC(const int & thread, const pt::ptree & config):p(config.get<double>("Momentum")), qc(config.get<double>("Q_Cutoff")), mu(config.get<double>("Chemical_Potential")), alpha(config.get<double>("Alpha")), relm(config.get<double>("Impurity_Mass")),
										...
										fw(config.get<double>("Fake_Weight")), fwzero(config.get<double>("Fake_Weight_Zero")), fwone(config.get<double>("Fake_Weight_One")), 
										qsigma(config.get<int>("Q_Gauss")), explim(config.get<double>("Exp_Arg_Limit")), doitlim(config.get<int>("Do_While_Limit")),
										fog0seg0(config.get<bool>("FOG0SEG0")), ct_taucut(config.get<bool>("CT_Tau_Cut")),ct_lin(config.get<bool>("CT_Tau_Linear")),ins_tau2cut(config.get<bool>("Ins_Tau2_Cut")),ins_tau2lin(config.get<bool>("Ins_Tau2_Linear")),ic_taulin(config.get<bool>("IC_Tau2_Linear")),ic_taucut(config.get<bool>("IC_Tau2_Cut")),
...
{ 
  try{
	...
	overflowstat = ArrayXXd::Zero(11,6);
