...
class tmap {
  ...
  public: 
	...
	const double taumin;
	const double taumax;
	const double taug0max;
	
	//create map
	tmap(const std::vector<std::function<double(int)> > & fvec, const std::vector<int> bins, const std::vector<double> taus, const int & tmit, const int & tmait);
