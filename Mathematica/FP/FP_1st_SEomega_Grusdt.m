...
g[q_, w_, t_]:= - (4*Pi)/(2*Pi)^4 * (q^2*vq2FP[q])/(I*w+\[Mu])/(I*w-q^2/2+\[Mu]-1)*Exp[- I * w* t];
g[1,-101,1]
Plot[Re[g[10,q,1]],{q,-qc,qc}]


g0setau[t_] :=2 * Re[NIntegrate[g[q,w, t],{q, 0, qc}, {w, 0, 100}]];
data= Table[{t,g0setau[t]}, {t, 0.5, 5, 0.25} ];


Export["peter/mat_1st_g0sew", data, "Table"];


g0setau[t_] :=NIntegrate[g[q,w, t],{q, 0, qc}, {w, -qc, qc}];
g0setau[1]


bl[M_, qc_]:=1- (8*Pi)/(3(2*Pi)^3)*1/M^2*NIntegrate[vq2FP[q]*q^2/(1+q^2/2/M), {q,0,qc}];
bl[1,1]


Mbl2[qc_]:= m /. FindRoot[bl[m,qc]-1/m, {m,0.5}] ;
g2[q_,qc_]:=(4*Pi)/(2*Pi)^3 * (q^2*vq2FP[q])/(-q^2/2/Mbl2[qc]-1);
epsi2[qc_]:=NIntegrate[g2[q,qc],{q,0, qc}];
epsi2[10000];
Plot[epsi2[qc],{qc,100,10000}]
